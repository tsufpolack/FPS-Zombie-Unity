...
public class CoverBehaviour : GenericBehaviour
{
	...
	void Update ()
	{
		...
		if (takeCover)
		{
			...
			// Align jump over cover direction (jump over is a transitional action to default behaviour).
	...
	void JumpOverMatchTarget()
	{
		...
		if (behaviourManager.GetAnim.GetCurrentAnimatorStateInfo(0).IsName("Turn180") && behaviourManager.GetAnim.updateMode != AnimatorUpdateMode.Fixed)
		{
			// Set update mode to animate physics for full rigidbody rotation.
			behaviourManager.GetAnim.updateMode = AnimatorUpdateMode.Fixed;
		}
		else if (behaviourManager.GetAnim.GetCurrentAnimatorStateInfo (0).IsName ("JumpOverCover"))
		{
			// Set update mode back to normal.
			if (behaviourManager.GetAnim.updateMode == AnimatorUpdateMode.Fixed)
				behaviourManager.GetAnim.updateMode = AnimatorUpdateMode.Normal;

			behaviourManager.GetAnim.MatchTarget (middlePoint, Quaternion.LookRotation (-coverWall.normal), AvatarTarget.Root, 
				new MatchTargetWeightMask (Vector3.one, 1f), 0.1f, 0.5f); 
		}
	}
	...
	{
		// Call pathfinding method for a possible path to the cover.
	...
	private void CheckForCorners()
	{
		...
		if(CanChangeCover(ray, out RaycastHit _))
		{
			possibleAction = CoverActions.CHANGE;
		}
		else if(CanTurnCover())
		{
	...
	private void DeadEndManagement(Ray ray)
	{
		...
		if (Physics.Raycast(ray.origin - shift, 2 * shift, out RaycastHit _, 2 * shift.magnitude))
		{
			behaviourManager.GetAnim.speed = 0f;
		}
		else
	...
	private void TurnCoverManagement()
	{
		...
		// Can turn the cover corner only when no path to other cover is available.
		if (!possibleCover)
		{
	...
	private bool HasPossibleCover(Vector3 rayTarget, out RaycastHit hit)
	{
		...
		if (hasCover)
		{
			if (Physics.Raycast(hit.point, Vector3.down, out RaycastHit groundHit, 3.0f))
